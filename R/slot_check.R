    ...
    for (su_i in used_su) {
        ...
        lapply(seq(nrow(su_cm)), function(obj_i) {
            ...
            if (any(meta[][, is.na(cell_ID)])) { 
                ...
                ID_col_guess <- which.max(sapply(meta[], 
                ...
                if (ID_col_guess == 0L) { 
                    ...
                    data.table::setnames(
                        meta[], old = ID_col_name, new = "cell_ID")
                    if (isTRUE(verbose)) {
                        wrap_msg(
        ...
        lapply(seq(nrow(ft_fm)), function(obj_i) {
            ...
            if (any(meta[][, is.na(feat_ID)])) { 
                ...
                if (ID_col_guess == 0L) { 
                    ...
                    data.table::setnames(
                        meta[], old = ID_col_name, new = "feat_ID")
                    if (isTRUE(verbose)) {
                        wrap_msg(
        ...
        for (coord_i in avail_sl[spat_unit == spat_unit_i, name]) {
            ...
            if (!isTRUE(missing_cell_IDs)) {
                ...
                if (!setequal(spatial_cell_id_names, expected_cell_ID_names)) {
                    stop("cell_IDs between spatial and expression information 
                ...
                if (spatlocsDT[, .N] != length(expected_cell_ID_names)) {
                    stop("Number of rows of spatial locations do not match 
                        with cell IDs for: \n spatial unit: ", 
