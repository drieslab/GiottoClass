...
#' @rdname shear
#' @export
setMethod("shear", signature("spatLocsObj"), function(x, fx = 0, fy = 0, x0, y0, ...) {
    a <- get_args_list(...)
    a$x <- x[]
...
#' @rdname shear
#' @export
setMethod("shear", signature("SpatVector"), function(x, fx = 0, fy = 0, x0, y0, ...) {
    a <- get_args_list(...)
...
#' @rdname shear
#' @export
setMethod("shear", signature("giottoLargeImage"), function(x, fx = 0, fy = 0, x0, y0, ...) {
    a <- get_args_list(...)
...
#' @rdname shear
#' @export
setMethod("shear", signature("giottoAffineImage"), function(x, fx = 0, fy = 0, x0, y0, ...) {
    a <- get_args_list(...)
    a$x <- x@affine
    # update affine
...
#' @rdname shear
#' @export
setMethod("shear", signature("affine2d"), function(x, fx = 0, fy = 0, x0, y0, ...) {
    a <- get_args_list(...)
...
.shear_dt <- function(x, fx = 0, fy = 0, x0, y0, geom = c("sdimx", "sdimy", "sdimz"), ...) {
    x <- data.table::copy(x)
    xyz <- tail(letters, 3L)
